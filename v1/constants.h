...
const float inv_radius_wheel  = 0.027778;  //in MILIMETERS-1
const byte dist_wheels = 252;         //distance between wheels in MILLIMETERS
const float inv_dist_wheels = 0.003968; //in MILIMETERS-1
...
int maxPWM_L = 150;                     //Define the minimum pwm value for the left wheel (up to 255), you could also diminish the voltage of the motor to keep a better control
...
// --- Regulator
// These coefficient should be modified to have the desired behaviour 
float Kp_rot = 2.7;
float Ki_rot = 2.3;
float Kd_rot = 0.4;
float Kp_length = 4;
float Ki_length = 2.3;
float Kd_length = 0.2;
const float Te = 0.1;            //Sampling time (in sec) for the PID
...
// --- Trajectory
const float T_ech = 0.1;        //Setpoint change frequency in void move_to_setpoint
